...
class Cooked_Recipes {
			...
			if ( !$printing && !$switcher_disabled ):
				...
				echo '<select name="servings" class="cooked-servings-changer">';
					foreach ( $servings_array as $stype ):
						echo '<option value="' . ( $default == $stype['value'] ? remove_query_arg( 'servings', get_permalink() ) : add_query_arg( 'servings', $stype['value'], get_permalink() ) ) . '"' . ( $stype['value'] == $servings ? ' selected' : '' ) . '>' . $stype['name'] . '</option>';
					endforeach;
				echo '</select>';
			else:
	...
	public static function difficulty_level( $level ){
		...
		return ( $level_text ? '<span class="cooked-difficulty-level-' . esc_attr( $level ) . '">' . $level_text . '</span>' : '' );
	}
